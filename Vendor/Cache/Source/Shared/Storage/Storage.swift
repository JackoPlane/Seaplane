...
extension Storage: StorageAware {
    public func entry(forKey key: String) throws -> Entry<T> {
        return try syncStorage.entry(forKey: key)
    }

    public func removeObject(forKey key: String) throws {
        try syncStorage.removeObject(forKey: key)
    }

    public func setObject(_ object: T, forKey key: String, expiry: Expiry? = nil) throws {
        try syncStorage.setObject(object, forKey: key, expiry: expiry)
    }

    public func removeAll() throws {
        try syncStorage.removeAll()
    }

    public func removeExpiredObjects() throws {
        try syncStorage.removeExpiredObjects()
    }
}
